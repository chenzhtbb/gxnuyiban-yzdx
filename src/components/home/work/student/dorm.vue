<template>
  <div class="dorm">
          ...
          <el-col :span="24" style="border-right: 1px solid #eee;">
            <router-link :to="link.form">
              <div class="m-auto p-3">
                ...
                <span style="font-size: 14px">外宿申请</span>
              </div>
            </router-link>
          </el-col>
        </el-row>
    ...
    <cube-scroll :data="applyDorm" style="position: fixed;left: 0; right: 0; top:154px;" :style="hei()">
      <div>
            ...
            <el-row v-for="(item, index) in applyDorm" :key="index" class="p-3 border-b">
              ...
              <el-col :span="21">
                <div @click="showModel(item)">
                  ...
                  <div class="dorm-type">
                    外宿住址：{{item.landlordAddress}}<br>
                    家长电话：{{item.tel2}}<br>
                    房东电话：{{item.landlordTel}}<br>
                    是否退宿：{{item.move}}<br>
                    当前进度：
                    <a>待辅导员审批</a>
...
<script>
  ...
  export default {
    ...
    activated() {
      this.applyDorm = getCache('applyDorm')
      this.myDorm = getCache('myDorm')
    },
    methods: {
      del(index) {
        this.applyDorm.splice(index, 1)
        setCache('applyDorm', this.applyDorm)
