<template>
  ...
  <transition appear name="v-img-fade">
    <div v-if="!closed" class="fullscreen-v-img" @click.self="close">
      ...
      <div class="content-v-img" @click="close">
        <slider :data="images"
                @next="next"
                @prev="prev"
        ...
        >
          <img
            ...
            @click="close"
            ref="img">
          <img v-if="flagstyle !== ''" :style="flagstyle">
        </slider>
      </div>
    </div>
  </transition>
...
<script type="text/ecmascript-6">
  import XScroll from 'base/scroll/x-scroll'
  ...
  export default {
    ...
    watch: {
      ...
      images () {
        this.$nextTick(() => {
          if (this.$refs.img) {
            this.flagstyle = `height: ${window.outerHeight}px; width: 0;`
          }
        })
      }
    },
